...
import { helperGetDataTreemap, setConfigChartTreemap } from './helper-treemap.component';
...
@Component({
  selector: '[pq-treemap-async]',
  templateUrl: './treemap2.component.html',
...
export class Treemap2Component implements OnInit {
  @Input() cardID: string;
  ...
  async ngOnInit() {
    ...
    if (!this.exploreJson) {
      ...
      let explore = helperGetDataTreemap(this.exploreJson, this._apicall);
  ...
  setFilter(param, explore) {
    ...
    param = { ...param, form_data: { ...param.form_data, since: this.sinceDate, until: this.untilDate } };
    ...
    if (array.length > 0) {
      for (let index = 0; index < array.length; index++) {
        ...
        for (let i = 0; i < params.treePathInfo.length; i++) {
          mydata[element.value] = [{ filter: element.key, id: split[splitIndex], metric: 2, text: split[splitIndex] }];
        }
      }
    }
  ...
  async getConfigChart(data) {
    ...
    if (!data) this.noData = true;
      ...
      config = await setConfigChartTreemap(
        data,
        this.mappingExtraFilter,
        this.explore,
        this.colorPalette,
